...
RUN cd /tmp; wget -O ejabberd.tgz https://www.process-one.net/downloads/downloads-action.php?file=/ejabberd/18.12/ejabberd-18.12.tgz
RUN cd /tmp; tar -xf ejabberd.tgz

RUN cd /tmp/ejabberd-18.12; ./configure  --disable-graphics --enable-user=ejabberd --enable-mysql --enable-pam --enable-zlib
RUN cd /tmp/ejabberd-18.12; make
RUN cd /tmp/ejabberd-18.12; make install
