...
RUN cd /tmp; wget -O ejabberd.tgz https://www.process-one.net/downloads/downloads-action.php?file=/ejabberd/19.02/ejabberd-19.02.tgz
RUN cd /tmp; tar -xf ejabberd.tgz

RUN cd /tmp/ejabberd-19.02; ./configure  --disable-graphics --enable-user=ejabberd --enable-mysql --enable-pam --enable-zlib
RUN cd /tmp/ejabberd-19.02; make
RUN cd /tmp/ejabberd-19.02; make install
